...
class FlowRateSetView: WKInterfaceController {
    ...
    override func willActivate() {
        ...
        inputData = 0
        valueSetView.setText(String(inputData) + UNT_W_DROP_FLOW_RATE)
        
        self.setTitle(TITLE_W_DROP_FLOW_RATE)
    ...
    func keyValueSet(keyValue: Int){
        ...
        case VAL_W_KEYBUTTON_OK:
            userDefaults.set(inputData, forKey: KEY_W_DROP_FLOW_RATE) // ここを変更する
            // 入力3項目「輸液量・所要時間・滴下速度」のうち、一番最新の入力にフラグを入れる
            LeastInputItem_W = LEAST_W_INPUT_DROP_FLOW_RATE
            pop()
