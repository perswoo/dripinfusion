//
//  FluidVolumeInputViewController.swift
//  DripHeart
//
...
class FluidVolumeInputViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, MFMailComposeViewControllerDelegate {
    
    
    @IBOutlet weak var fluidVolumeInputTableView: UITableView!
    ...
    override func viewDidLoad() {
        ...
        fluidVolumeInputTableView.register(UINib(nibName: "MainTableViewCell", bundle: nil), forCellReuseIdentifier: "customCell")

        if let storedData = UserDefaults().data(forKey: KEY_FLUID_VOLUME_INPUT_LIST) {
            do {
                let unarchivedData = try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(storedData)
                fluidVolumeInputList.append(contentsOf: unarchivedData as! [FluidVolumeInputList])
            } catch {
                print(error)
            }
        }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        // スクロールインジケータを一時的に表示
        self.fluidVolumeInputTableView.flashScrollIndicators()
    }
    ...
    *
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return fluidVolumeInputList.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let fluidVolumeInputList = fluidVolumeInputList[indexPath.row]
        
        cell.textLabel?.text = fluidVolumeInputList.fluidVolumeInputValue! + UNT_DROP_FLUID_VOLUME
        
        if (fluidVolumeInputList.fluidVolumeInputValue == userDefaults.string(forKey: KEY_DROP_ALL_AMOUNT)){
            // チェックマークを入れる
    ...
    internal func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let dataSet = fluidVolumeInputList[indexPath.row].fluidVolumeInputValue {
            // トップ画面の液量記録用
            userDefaults.set(dataSet, forKey: KEY_DROP_ALL_AMOUNT)
            
            // 入力3項目「輸液量・所要時間・滴下速度」のうち、一番最新の入力にフラグを入れる
            LeastInputItem = LEAST_INPUT_DROP_FLUID_VOLUME
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            fluidVolumeInputList.remove(at: indexPath.row)
            tableView.deleteRows(at: [indexPath], with: .fade)

            let userDefalults = UserDefaults.standard
            do {
                let data: Data = try NSKeyedArchiver.archivedData(withRootObject: self.fluidVolumeInputList, requiringSecureCoding: false)
                userDefalults.set(data, forKey: KEY_FLUID_VOLUME_INPUT_LIST)
    ...
    func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        let moveRowData = fluidVolumeInputList[sourceIndexPath.row]
        fluidVolumeInputList.remove(at: sourceIndexPath.row)
        fluidVolumeInputList.insert(moveRowData, at: destinationIndexPath.row)
    ...
    */
    ...
    @IBAction func addButton(_ sender: Any) {
        ...
        // トップ画面の液量の登録
        userDefaults.set(finalTextFieldAsString, forKey: KEY_DROP_ALL_AMOUNT)
        
        
        // [課題]　テキストと履歴リストが一致する際には履歴リストの登録を行わない
        //if (finalTextFieldAsString != userDefaults.string(forKey: KEY_DROP_ALL_AMOUNT)){
        //}
        
        
        // 履歴リストの登録
        let fluidVolumeInputList = FluidVolumeInputList()
        fluidVolumeInputList.fluidVolumeInputValue = finalTextFieldAsString
        self.fluidVolumeInputList.insert(fluidVolumeInputList, at: 0)

        //self.fluidVolumeInputTableView.insertRows(at: [IndexPath(row: 0, section: 0)], with: .right)
        ...
        let userDefaults = UserDefaults.standard
        do {
            let archivedData: Data = try NSKeyedArchiver.archivedData(withRootObject: self.fluidVolumeInputList, requiringSecureCoding: false)
            userDefaults.set(archivedData, forKey: KEY_FLUID_VOLUME_INPUT_LIST)
